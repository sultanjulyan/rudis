...
function Rewrite-Paths {
    ...
    $Content = $Content -replace '(/?)memory/', '.rudis/memory/'
    $Content = $Content -replace '(/?)scripts/', '.rudis/scripts/'
    $Content = $Content -replace '(/?)templates/', '.rudis/templates/'
    return $Content
}
...
function Generate-Commands {
    ...
    foreach ($template in $templates) {
        ...
        $outputFile = Join-Path $OutputDir "rudis.$name.$Extension"
...
function Generate-CopilotPrompts {
    ...
    $agentFiles = Get-ChildItem -Path "$AgentsDir/rudis.*.agent.md" -File -ErrorAction SilentlyContinue
...
function Build-Variant {
    ...
    $specDir = Join-Path $baseDir ".rudis"
    New-Item -ItemType Directory -Path $specDir -Force | Out-Null
    
    # Copy memory directory
    if (Test-Path "memory") {
        Copy-Item -Path "memory" -Destination $specDir -Recurse -Force
        Write-Host "Copied memory -> .rudis"
    }
    ...
    if (Test-Path "templates") {
        ...
        Write-Host "Copied templates -> .rudis/templates"
    }
    ...
    # Create zip archive
    $zipFile = Join-Path $GenReleasesDir "rudis-template-${Agent}-${Script}-${Version}.zip"
...
Get-ChildItem -Path $GenReleasesDir -Filter "rudis-template-*-${Version}.zip" | ForEach-Object {
    Write-Host "  $($_.Name)"
}